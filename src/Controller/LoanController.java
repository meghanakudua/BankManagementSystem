...
public class LoanController {
    ...
    public static List<Loan> getLoansByAccountNumber(String accountNumber) {
        ...
        return loans;  // Return the list of loans for the specified account
    }


    // Monthly EMI deduction & fine application logic
    /*public static void processLoanPayments() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                // Check if one month has passed since last payment
                long monthsSinceLastPayment = ChronoUnit.MONTHS.between(lastPayment, now);

                // Only proceed with payment if a full month has passed
                if (monthsSinceLastPayment > 0) {
                    int missedPayments = (int) monthsSinceLastPayment;

                    // Apply fines for missed payments
                    fine += missedPayments * 1000;  // Fine for each missed payment month

                    // Now check customer balance
                    ...
                    try (PreparedStatement balanceStmt = conn.prepareStatement(balanceQuery)) {
                        ...
                        if (balanceRs.next()) {
                            ...
                            if (balance >= emi) {
                                ...
                                fine += 1000;  // Fine for this month if balance is too low
                            }
                        }
                    }

                    // Check if loan is fully paid
                    String newStatus = totalPaid >= loanAmount ? "Completed" : "Active";

                    // Update loan record
                    String updateLoan = "UPDATE loans SET total_paid = ?, fine = ?, status = ?, last_payment_date = ? WHERE loan_id = ?";
                    try (PreparedStatement updateLoanStmt = conn.prepareStatement(updateLoan)) {
                        ...
                        updateLoanStmt.setString(3, newStatus);
                        updateLoanStmt.setDate(4, Date.valueOf(now));  // Update last payment date to current date
                        updateLoanStmt.setInt(5, loanId);
