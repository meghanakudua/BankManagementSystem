...
public class BankStaffController {
    ...
    /*public List<Transaction> getBranchTransactions(String branch) {
        ...
        String query = """
        SELECT t.account_number, t.type, t.from_account, t.to_account, t.timestamp, c.balance
        FROM transactions t
    ...
    /*public List<Transaction> getBranchTransactions(String branch) {
        List<Transaction> transactions = new ArrayList<>();
        String query = """
        SELECT t.account_number, t.type, t.description, t.timestamp, c.balance
        FROM transactions t
        JOIN customers c ON t.account_number = c.account_number
        WHERE c.branch = ?
        ORDER BY t.timestamp DESC
    """;
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                if (type.equals("account transfer") && description != null) {
                    if (description.startsWith("to ")) {
                        to = description.substring(3);
                    } else if (description.startsWith("from ")) {
                        from = description.substring(5);
                    }
                }


                transactions.add(new Transaction(accountNumber, type, from, to, timestamp, balance));
            }
    ...
    /*public List<Transaction> getBranchTransactions(String branch) {
        List<Transaction> transactions = new ArrayList<>();
        try (Connection conn = DBConnection.getConnection()) {
            String sql = "SELECT t.account_number, t.type, t.amount, t.timestamp, t.related_account " +
                    "FROM transactions t " +
                    "JOIN customers c ON t.account_number = c.account_number " +
                    "WHERE c.branch = ? " +
                    "ORDER BY t.timestamp DESC";
            ...
            while (rs.next()) {
                ...
                transactions.add(new Transaction(
                        accountNumber, type, from, to, timestamp, amount // <- pass amount as the last param (if that's balance)
                ));
